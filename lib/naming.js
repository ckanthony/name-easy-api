...
const getSynonymsAtThesaurus = async function (word) {
  ...
  return _.shuffle(candidates.map(c => ({ verb: word, result: `i${c}U` })));
}
...
const getSynonyms = function (word, p = null) {
  ...
  if (results.v.includes('v')) {
    ...
    if (candidates.length > 0) {
      return candidates.map(c => ({ verb: word, result: `i${c}U` }));
    }
    return [{ verb: word, result: `i${word}U` }];
  }
  console.log(results)
  return _.shuffle(results.v.map(v => ({ verb: word, result: `i${v}U` })));
}
