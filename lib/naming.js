...
const naming = async function (mission) {
  try {
    ...
    // console.log(refine)
    let results = await Promise.all(verbs.map(async r => await getSynonymsAtThesaurus(r)));
    console.log(results)
    let resultList = [];
    verbs.forEach((v, i) => {
      if (results[i]) {
        resultList.push({verb: v, result: `i${results[i].charAt(0).toUpperCase() + results[i].slice(1)}U`});
      }
    })
    return resultList;
  } catch (e) {
...
const getSynonyms = function (word, p = null) {
  ...
  console.log(results)
  return results.v[Math.floor(Math.random() * results.v.length)];
}
