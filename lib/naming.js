...
const naming = async function (mission) {
  try {
    ...
    return list.map(l => ({ verb: l.verb, result: `i${l.result.charAt(0).toUpperCase() + l.result.slice(1)}U` }));
  } catch (e) {
    console.error(e);
    return false; 
  }
...
const getSynonyms = function (word, p = null) {
  ...
  if (word === 'fuck') { return [{ verb: word, result: 'deal' }]; }
  console.log(`testinghere: ${word}, results:`, results)
  if (!results && !results.v && !results.n) { return false; }
  let candidates = []
  if (results.n && results.n.length) { candidates = candidates.concat(results.n); }
  if (results.v && results.v.length) { candidates = candidates.concat(results.v); }
  // if (results.v.includes('v')) {
  //   console.log('=======')
  //   console.log(results.v.includes('v'))
  ...
  // }
  console.log(candidates)
  return _.shuffle(candidates.map(v => ({ verb: word, result: v })));
}
